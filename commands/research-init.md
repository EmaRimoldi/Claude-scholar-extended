...
- `hypotheses.md` → `$PROJECT_DIR/docs/hypotheses.md` (generated by hypothesis-generator agent)
- `datasets.md` → `$PROJECT_DIR/docs/datasets.md`
...
### Step 4: Literature Synthesis (SEPARATE from search)

You are now writing synthesis. Do NOT search for more papers. Work only from what was found in Steps 2-3.

Principle: The searcher finds, the synthesizer interprets. Do not mix these roles.
...
4. **datasets.md** - Dataset catalog (always generated)

### Dataset Catalog Format

Generate `$PROJECT_DIR/docs/datasets.md` listing ALL datasets discovered during literature review:
...
| **Size** | [N examples, disk size] |
| **Tasks** | [what tasks it supports] |
| **License** | [license type] |
| **Relevance** | [why relevant to our research] |
| **Limitations** | [known issues or biases] |

List all datasets found, not just those chosen for experiments.

### Per-Paper Annotation Format

For each paper in the literature review, include:
- **Citation**: [Authors, Year, Title, Venue]
- **Key Contribution** (1 sentence): [what this paper adds]
- **Relevance to Our Work** (2-3 sentences): [why it matters for our research question]
- **Tags**: [method type, task domain, key technique]
...
└── docs/
    ...
    └── datasets.md               # Dataset catalog (all discovered datasets)
```
...
5. Invoke the **hypothesis-generator agent** (Opus) for hypothesis generation from the literature review
