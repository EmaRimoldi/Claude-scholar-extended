...
echo "Starting C4 (sparsemax+MSE all-12 heads — PRIMARY) seed=$SEED on $(hostname)"
mkdir -p logs checkpoints/C4/seed_$SEED

$VENV train.py \
    model=bert_sparsemax \
    seed=$SEED \
    condition=C4 \
    training.output_dir=checkpoints/C4/seed_$SEED \
    training.device=cuda \
    +training.run_id=C4_seed${SEED}

echo "C4 seed=$SEED complete"
