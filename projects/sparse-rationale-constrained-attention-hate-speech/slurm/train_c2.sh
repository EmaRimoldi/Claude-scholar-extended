...
echo "Starting C2 (SRA replication) seed=$SEED on $(hostname)"
mkdir -p logs checkpoints/C2/seed_$SEED

$VENV train.py \
    model=bert_softmax_sra \
    seed=$SEED \
    condition=C2 \
    training.output_dir=checkpoints/C2/seed_$SEED \
    training.device=cuda \
    +training.run_id=C2_seed${SEED}

echo "C2 seed=$SEED complete"
